...
public class Warrior {
	...
	// Monster doBattle()을 호출해 자기 데미지를 몬스터에게 넘겨주고
	...
	public void hunt(Monster monster) {
		if (monster.getHp() <= 0) { // 호출전 몬스터가 죽어있는지 살아있는지 확인하는 구문
			System.out.println("해당 " + monster.getName() + "는 죽었습니다."); 
			return;
		}
		monster.doBattle(atk);  // 유저가 몬스터를 선공
		
		if (monster.getHp() <= 0) {
			System.out.println("전사가 " + monster.getName() + "를 처치했습니다");
			System.out.println("경험치를 획득합니다. 경험치 : " + monster.getExp());
		} else {
			this.hp -= monster.getAtk();  // 자신의 체력에서 몬스터 공격력만큼 차감
		}
